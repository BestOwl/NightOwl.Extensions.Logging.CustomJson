...
namespace NightOwl.Extensions.Logging.CustomJson
{
    internal sealed class JsonConsoleFormatter : ConsoleFormatter, IDisposable
    {
        ...
        private static readonly Dictionary<LogLevel, string> DefaultLogLevelNames = new()
        {
            ...
            DateTimeOffset stamp)
        {
            ...
            using (var output = new PooledByteBufferWriter(DefaultBufferSize))
            {
                using (var writer = new Utf8JsonWriter(output, FormatterOptions.JsonWriterOptions))
                {
                    ...
                    {
                        writer.WriteString(FormatterOptions.TimestampFieldName ?? "Timestamp", stamp.ToString(timestampFormat));
                    }
                    writer.WriteNumber(FormatterOptions.EventIdFieldName ?? nameof(LogEntry<object>.EventId), eventId);
                    writer.WriteString(FormatterOptions.LogLevelFieldName ?? nameof(LogEntry<object>.LogLevel), GetLogLevelString(logLevel));
                    writer.WriteString(FormatterOptions.CategoryFieldName ?? nameof(LogEntry<object>.Category), category);
                    writer.WriteString(FormatterOptions.MessageFieldName ?? "Message", message);
 
                    if (exception != null)
                    {
                        writer.WriteString(FormatterOptions.ExceptionFieldName ?? nameof(Exception), exception);
                    }
        ...
        {
            if (FormatterOptions.LogLevelNameMapping != null && 
                FormatterOptions.LogLevelNameMapping.TryGetValue(logLevel, out var customName))
            {
                return customName;
            }

            return DefaultLogLevelNames.TryGetValue(logLevel, out var defaultName) 
                ? defaultName 
        ...
        internal CustomJsonConsoleFormatterOptions FormatterOptions { get; set; }
